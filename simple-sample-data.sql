-- ===============================================
-- DATACRM - CORRECTED SAMPLE DATA INSERT SCRIPT
-- Creates realistic sample data with valid UUIDs
-- Version: 2.1
-- Date: 2025-07-31
-- ===============================================

-- First, let's generate some valid UUIDs using gen_random_uuid() or use predefined valid ones
...
-- Store organization IDs in variables for later use
DO $$
DECLARE
    techflow_org_id UUID;
    creative_org_id UUID;
    bec_org_id UUID;
    super_admin_id UUID := gen_random_uuid();
    techflow_ceo_id UUID := gen_random_uuid();
    techflow_sales_id UUID := gen_random_uuid();
    techflow_dev_id UUID := gen_random_uuid();
    creative_owner_id UUID := gen_random_uuid();
    creative_designer_id UUID := gen_random_uuid();
    bec_partner_id UUID := gen_random_uuid();
BEGIN
    -- Get organization IDs
    SELECT id INTO techflow_org_id FROM organizations WHERE slug = 'techflow-solutions';
    SELECT id INTO creative_org_id FROM organizations WHERE slug = 'creative-marketing-hub';
    SELECT id INTO bec_org_id FROM organizations WHERE slug = 'business-excellence';
...
SELECT 
    ...
    (SELECT count(*) FROM organizations WHERE id NOT LIKE '00000000-0000-0000-0000-000000000000') as sample_organizations,
